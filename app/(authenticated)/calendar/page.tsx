...
export default function CalendarPage() {
    ...
    const fetchData = async () => {
      try {
        const [schedulesRes, absencesRes] = await Promise.all([
          fetch(`${process.env.NEXT_PUBLIC_GAS_API_URL}?path=schedules`),
          fetch(`${process.env.NEXT_PUBLIC_GAS_API_URL}?path=absences`),
        ]);
        ...
        if (schedulesData.success) {
          ...
          setError(schedulesData.error || "データの取得に失敗しました");
        }
  ...
  schedules.forEach((schedule) => {
    ...
    if (startYear && startMonth && startDay) {
      ...
      while (currentDate <= endDate) {
        ...
        const isWeekStart =
          dayOfWeek === 0 || currentDate.getTime() === startDate.getTime();
        const isWeekEnd =
          dayOfWeek === 6 || currentDate.getTime() === endDate.getTime();
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "スケジュールの追加に失敗しました",
      );
    } finally {
  ...
  const openEditModal = () => {
    ...
    const hasEndDate =
      rawEndYear !== "" && rawEndYear !== null && rawEndYear !== undefined;
    ...
    setEditForm({
      ...
      timeHH:
        rawTimeHH !== "" && rawTimeHH !== null && rawTimeHH !== undefined
          ...
          : "",
      timeMM:
        rawTimeMM !== "" && rawTimeMM !== null && rawTimeMM !== undefined
          ? String(rawTimeMM)
          : "",
      ...
      endYear:
        rawEndYear !== "" && rawEndYear !== null && rawEndYear !== undefined
          ...
          : "",
      endMonth:
        rawEndMonth !== "" && rawEndMonth !== null && rawEndMonth !== undefined
          ? String(rawEndMonth)
          : "",
      endDate:
        rawEndDate !== "" && rawEndDate !== null && rawEndDate !== undefined
          ? String(rawEndDate)
          : "",
      isAllDay,
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "スケジュールの削除に失敗しました",
      );
    } finally {
      setIsDeleting(false);
    }
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "スケジュールの更新に失敗しました",
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const getEventTime = (schedule: Schedule): string | null => {
    ...
    const hasTime =
      ...
      rawTimeMM !== undefined;
    return hasTime
      ? `${String(rawTimeHH).padStart(2, "0")}:${String(rawTimeMM).padStart(
          2,
          "0",
        )}`
      : null;
    ...
    <div className="max-lg:p-0 lg:p-6 w-full lg:h-full flex flex-col lg:items-stretch items-center bg-base-100 lg:overflow-hidden">
              ...
              return (
                ...
                >
                  ...
                  <div className="relative flex-1">
                    {day.events.slice(0, 3).map((eventWithPos, eventIndex) => {
                      const title = getEventTitle(eventWithPos.schedule);
                      const { position, isWeekStart, isWeekEnd } = eventWithPos;
                      const isMultiDay = position !== "single";

                      // イベントの色を取得
                      const eventColor = getColorHex(
                        String(
                          Object.values(eventWithPos.schedule)[12] || "primary",
                        ),
                      );
                      ...
                      const topOffset = eventIndex * 16; // 各イベントの高さ + gap
              ...
              >
                ...
                <div className="relative flex-1">
                  {day.events.slice(0, 2).map((eventWithPos, eventIndex) => {
                    const time = getEventTime(eventWithPos.schedule);
                    const title = getEventTitle(eventWithPos.schedule);
                    const { position, isWeekStart, isWeekEnd } = eventWithPos;
                    const isMultiDay = position !== "single";
        ...
        <dialog className="modal modal-open modal-middle">
          <div className="modal-box max-w-2xl max-h-[calc(100vh-5rem)] flex flex-col">
            ...
            <h3 className="font-bold text-xl mb-4">
              ...
              {selectedDate.date.getMonth() + 1}月{selectedDate.date.getDate()}
              日の予定
              ...
              {selectedDate.events.length === 0 ? (
                ...
                <div className="space-y-3">
                  {selectedDate.events.map((eventWithPos, index) => {
                    const { schedule, position } = eventWithPos;
                    const values = Object.values(schedule);
                    const startYear = Number(values[1]);
                    const startMonth = Number(values[2]);
                    const startDay = Number(values[3]);
                    ...
                    const dateRangeLabel =
                      isMultiDay && endYear && endMonth && endDay
                    ...
                    return (
                      <div
                        key={index}
                        onClick={() => handleEventSelect(schedule)}
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAddForm({
                        ...
                        // 終日をオンにしたら時刻をクリア
                        timeHH: e.target.checked ? "" : addForm.timeHH,
                        timeMM: e.target.checked ? "" : addForm.timeMM,
                      })
                    }
                  />
              ...
              <div className="modal-action">
                ...
                <button
                  ...
                  disabled={isSubmitting || !addForm.title.trim()}
                >
                  {isSubmitting ? (
      ...
      {selectedDate &&
        ...
        (() => {
          ...
          const scheduleDayOfWeek = ["日", "月", "火", "水", "木", "金", "土"][
            scheduleDate.getDay()
          ];
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditForm({
                        ...
                        isAllDay: e.target.checked,
                        // 終日をオンにしたら時刻をクリア
                        timeHH: e.target.checked ? "" : editForm.timeHH,
                        timeMM: e.target.checked ? "" : editForm.timeMM,
                      })
                    }
                  />
                  <span className="label-text">終日</span>
