...
export default async function HomePage() {
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-10 max-w-full">
            <h1 className="font-bold mb-8 max-lg:hidden" style={{ fontSize: 'clamp(1.5rem, 4vw, 2.25rem)' }}>
                ダッシュボード
            </h1>
            {error && (
                ...
                <div className="card bg-base-100 shadow-xl hover:shadow-2xl transition-shadow border border-base-300 h-[420px] order-1">
                    <div className="card-body pt-5 flex flex-col overflow-hidden">
                        <div className="flex items-center gap-2 mb-4 h-8 shrink-0">
                            ...
                            <h2 className="card-title" style={{ fontSize: 'clamp(1rem, 2.5vw, 1.25rem)' }}>
                                今後のスケジュール
                            </h2>
                        ...
                        {upcomingSchedules.length > 0 ? (
                            <div className="flex-1 overflow-y-auto pr-2 space-y-4 min-h-0">
                                {upcomingSchedules.map(
                                    (schedule: Schedule, index: number) => {
                                        ...
                                        const eventAbsences = absences.filter(
                                            (absence) => {
                                                const absenceValues =
                                                    Object.values(absence);
                                                return (
                                                    absenceValues[1] === eventId
                                                ); // B列のEVENT_IDで比較
                                            }
                                        );
                            ...
                            <div className="flex-1 flex items-center justify-center">
                                <div className="text-center text-base-content/60">
                                    ...
                                    <p style={{ fontSize: 'clamp(0.875rem, 2vw, 1.125rem)' }}>予定はありません</p>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div className="card bg-base-100 shadow-xl hover:shadow-2xl transition-shadow border border-base-300 order-2 lg:order-3 lg:col-span-2">
                    <div className="card-body pt-5">
                        <div className="flex items-center gap-2 mb-4 h-8">
                            ...
                            <h2 className="card-title" style={{ fontSize: 'clamp(1.25rem, 3vw, 1.5rem)' }}>
                                本日の欠席者
                            </h2>
                        </div>
                        {absences.length > 0 ? (
                            <div className="overflow-x-auto">
                                <table className="table table-zebra w-full" style={{ fontSize: 'clamp(0.875rem, 2vw, 1.125rem)' }}>
                                    <thead>
                                        <tr>
                                            <th>学籍番号</th>
                                            <th>氏名</th>
                            ...
                            <div className="text-center py-12">
                                ...
                                <p className="text-base-content/60" style={{ fontSize: 'clamp(1rem, 2.5vw, 1.25rem)' }}>
                                    本日の欠席者はいません
                                </p>
                            </div>
                        )}
