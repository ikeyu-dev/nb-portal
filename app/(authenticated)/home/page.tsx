...
export default async function HomePage() {
    ...
    const todayTimestamp = new Date(todayYear, todayMonth - 1, todayDate).getTime();
    ...
    return (
        <div className="p-10 max-w-full">
            <h1 className="text-4xl font-bold mb-8 max-lg:hidden">ダッシュボード</h1>
            {error && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        {upcomingSchedules.length > 0 ? (
                            <div className="flex-1 overflow-y-auto pr-2 space-y-4 min-h-0">
                                {upcomingSchedules.map((schedule: Schedule, index: number) => {
                                    ...
                                    const scheduleDayOfWeek = ["日", "月", "火", "水", "木", "金", "土"][scheduleDate.getDay()];
                                    const dateLabel = `${month}/${date}(${scheduleDayOfWeek})`;

                                    // 時刻ラベルを作成（値が存在する場合のみ）
                                    const hasTime = rawTimeHH !== "" && rawTimeHH !== null && rawTimeHH !== undefined &&
                                                   rawTimeMM !== "" && rawTimeMM !== null && rawTimeMM !== undefined;
                                    const timeLabel = hasTime
                                        ? `${String(rawTimeHH).padStart(2, "0")}:${String(rawTimeMM).padStart(2, "0")}`
                                        : undefined;
                ...
                <div className="card-body pt-5">
                    <div className="flex items-center gap-2 mb-4 h-8">
                        ...
                        <h2 className="card-title text-2xl">本日の欠席者</h2>
                    </div>
                    {absences.length > 0 ? (
                        <div className="overflow-x-auto">
                            <table className="table table-zebra w-full">
                                <thead>
                                    <tr>
                                        {Object.keys(absences[0]).map((key) => (
                                            <th key={key}>{key}</th>
                                        ))}
                                    </tr>
                                </thead>
